...
public class Sprite {
	...
	public Sprite(Image image , int radius) {
		this.image = image;
		positionX = 100;
		positionY = 500;
...
private int pic;
	...
	/*public void updateLocations() {
		double newPositionY = positionY -LevelType.fruitSpeedY;
		this.positionX += LevelType.fruitSpeedX;
		if(direction == -1) {
			if(newPositionY <maxHeight) {
				direction = 1;
				this.positionY += LevelType.fruitSpeedY;
			}
			else 
				this.positionY -= LevelType.fruitSpeedY;
		}
	...
	/*public Rectangle rect(GraphicsContext gc) {
		Rectangle rect = new Rectangle(circle.getTranslateX() , circle.getTranslateY() , image.getWidth() , image.getHeight());
...
*	QuadCurveTo quadCurve = new QuadCurveTo();
	...
	/*	Circle circ = new Circle(random.nextInt(100)+250);
		circ.setCenterX(400);
		circ.setCenterY(550);
		pathT.setPath(circ);
		pt2.setPath(circ);
	...
	//	pathT.setPath(path);
		...
		pathT.setDuration(Duration.millis(pathFruitDur));
		//path for circle
		pt2.setNode(objects.get(i).getCircle());
		pt2.setAutoReverse(false);
		pt2.setCycleCount(1);
		pt2.setDuration(Duration.millis(pathFruitDur));
		pathT.play();
		pt2.play();
